...
namespace NagyValtozasuTelepulesek
{
    class Program
    {
        ...
        {
            // BEOLVASÁS
            int[][] idojaras = Beolvas();
            int n = idojaras.Length;

            // FELDOLGOZÁS
            int[] y = Mintak.Kivalogat(0, n - 1, 
                i => TobbMintTiz(idojaras[i]), 
                i => i + 1);
        ...
        {
            if (napok.Length < 2) return false;

            return Mintak.Van(0, napok.Length - 2, 
                j => Math.Abs(napok[j + 1] - napok[j]) >= 10);
        }
